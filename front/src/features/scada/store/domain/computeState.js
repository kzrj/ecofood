...
function createCtx(initialSkus) {
  return {
    // Фоллбек “sku -> recipe”, чтобы красить SKU/рамы даже если в event нет recipe
    skuRecipeMap: new Map(initialSkus.map(({ id, recipe }) => [id, recipe])),
    // Суммарный план (вес всех SKU) — используем для подписи склада
...
function deriveViewModel(state, initialSkus) {
  ...
  if (initialSkus.length > 0) {
    const skuRows = initialSkus.map(({ id, recipe, weight: planned }) => ({
      sku: id,
      recipe,
      weight: state.skladBySku.get(id) ?? 0,
      planned,
    }))
